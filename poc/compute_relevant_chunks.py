...
# ---------------------------------------------------------------------
# LLM call: classify ALL chunks and compute metrics at once
# ---------------------------------------------------------------------
def classify_relevance_and_metrics(
    client: OpenAI,
    query: str,
    chunks: List[str],
    expected_answer: str,
    ...
    """
    Ask the LLM to:
      - decide which chunks are relevant to answer the query,
      - compute faithfulness, factual_recall, factual_precision, context_recall.

    Returns:
        (relevant_indices, faithfulness, factual_recall, factual_precision, context_recall)
    """

    # Build a numbered list of chunks for the prompt
    ...
    system_msg = (
        ...
        "Your tasks:\n"
        "1) Identify which chunks are RELEVANT to answer the query.\n"
        "   - A chunk is RELEVANT if it contains any information that helps answer the query,\n"
        "     even partially (facts, definitions, conditions, numeric values, examples, etc.).\n"
        "   - A chunk is NOT relevant if it only contains cookie banners, navigation menus,\n"
        "     generic boilerplate, marketing text, or content unrelated to the query.\n"
        "   - Consider the ENTIRE list of chunks; do not limit yourself to only the first few.\n\n"
        "2) Compute a FAITHFULNESS score (0 to 1) for the actual answer with respect to\n"
        "   the RELEVANT chunks: how grounded the answer is in those chunks.\n"
        ...
        "     relevant chunks (it may be a paraphrase, but must not contradict the chunks).\n"
        ...
        "   - If the answer is completely based on the relevant chunks, the score should\n"
        "     be 1.0.\n"
        "   - If the answer ignores those chunks completely, the score should be 0.0.\n\n"
        ...
        "5) Compute a CONTEXT_RECALL score (0 to 1) for the RELEVANT chunks with respect\n"
        "   to the user query: how sufficient the relevant chunks are to answer the query.\n"
        ...
        "   - Let C_covered = number of those requirements for which the RELEVANT chunks\n"
        ...
        "   - 1.0 means the relevant chunks, taken together, contain enough information\n"
        "     to fully answer the query.\n"
        "   - 0.0 means the relevant chunks do not provide useful information for any\n"
        ...
        "{\n"
        "  \"relevant_indices\": [i1, i2, ...],\n"
        "  \"faithfulness\": 0.0-1.0,\n"
        "  \"factual_recall\": 0.0-1.0,\n"
        "  \"factual_precision\": 0.0-1.0,\n"
        "  \"context_recall\": 0.0-1.0\n"
        "}\n"
        "where i1, i2, ... are 0-based integers and each index must be between 0 and N-1,\n"
        "N = number of chunks.\n"
    )
    ...
    user_msg = (
        ...
        "Actual answer:\n"
        f"{actual_answer}\n\n"
        "Decide which chunks are relevant and compute the four scores.\n"
    ...
    try:
        ...
        factual_recall = get_score("factual_recall")
        factual_precision = get_score("factual_precision")
        context_recall = get_score("context_recall")

    except json.JSONDecodeError:
        # On parse error: treat as no relevant chunks and all scores 0.0
        indices = []
        faithfulness = 0.0
        factual_recall = 0.0
        factual_precision = 0.0
    ...
    # Clean up indices: unique, sorted, and in range
    n = len(chunks)
    valid_indices = sorted(
        {i for i in indices if isinstance(i, int) and 0 <= i < n}
    )

    return valid_indices, faithfulness, factual_recall, factual_precision, context_recall
