...
The script evaluate_prompt_compliance.py strictly evaluates the to which extent the LLM answers comply with the instructions in the prompt descriptions. The results are stored in a json file. In addition, prompt improvements are also proposed with explanations. The compliance is graded on a scale {1, 2, 3, 4, 5} where 1 means the poorest compliance and the 5 the best compliance. The numeric assessment is accompanied with textual explanations. 